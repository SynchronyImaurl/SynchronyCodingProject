...
public class ImgurlImageService {
    ...
    public void updateImage(String imageId, String title, String description) throws UnirestException {

        HttpResponse<String> response = Unirest.post("https://api.imgur.com/3/image/" + imageId)
                .header("Authorization", "Client-ID 6f7d71c05691caa")
                .field("title", title)
        ...
        if (response.getStatus() != 200) {
            throw new IllegalArgumentException("Updating image with id "
                    + imageId + " failed.\nResponse from Imgurl api:\n" + response.getBody());
        }
    }
